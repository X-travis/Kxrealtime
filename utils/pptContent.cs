...
namespace kxrealtime.utils
{
    ...
    public static class pptContent
    {
        ...
        public static void InserVideo(string videlUrl)
        {
            ...
            objText.Text = videlUrl;
            objText.ActionSettings[Microsoft.Office.Interop.PowerPoint.PpMouseActivation.ppMouseClick].Hyperlink.Address = videlUrl;
        }
        ...
        //仅获取在线链接,无法竞争到线程，弃用
        public static void onlineFile(string pathTmp, string fileName, string type, SHOWPROGRESS cb, ProgressTip pgCb)
        {
            cb(true);
            var savePath = Utils.getFilePath();
            var filePath = savePath + @"\" + fileName;
